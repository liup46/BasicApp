...
import com.basic.util.BrandUtils
...
object StatusBarHelper {
    ...
    fun translucent(window: Window, @ColorInt colorOn5x: Int): Unit {
        ...
        if (BrandUtils.isFlymeLowerThan8 || BrandUtils.isMIUI &&
...
//            }
    ...
    fun supportTransclentStatusBar6(): Boolean {
        return !(BrandUtils.isZUKZ1 || BrandUtils.isZTKC2016)
    }
    ...
    private fun supportTranslucent(): Boolean {
        ...
        return (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT
                && !(BrandUtils.isEssentialPhone && Build.VERSION.SDK_INT < 26))
    }
    ...
    private fun initStatusBarHeight(context: Context?) {
        try {
            ...
            if (BrandUtils.isMeizu) {
                resId =
                    context?.resources?.getIdentifier("status_bar_height_large", "dimen", "android")
            }
            if (resId == null || resId < 0) {
                resId = context?.resources?.getIdentifier("status_bar_height", "dimen", "android")
            }
            if (resId != null && resId > 0) {
                sStatusBarHeight = context?.resources?.getDimensionPixelSize(resId) ?: 0
            }
        } catch (t: Throwable) {
            t.printStackTrace()
        }
        ...
        if (sStatusBarHeight <= 0) {
            if (sVirtualDensity == -1f) {
                sStatusBarHeight = STATUS_BAR_DEFAULT_HEIGHT_DP.dp.toInt()
            } else {
    ...
    private fun Android6SetStatusBarLightMode(window: Window, light: Boolean): Boolean {
        ...
        if (BrandUtils.isMIUIV9) { // MIUI 9 低于 6.0 版本依旧只能回退到以前的方案
    ...
    private fun isMIUICustomStatusBarLightModeImpl(): Boolean {
        return if (BrandUtils.isMIUIV9 && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            true
        } else BrandUtils.isMIUIV5 || BrandUtils.isMIUIV6 ||
                BrandUtils.isMIUIV7 || BrandUtils.isMIUIV8
    }
